...
dhtmlXTreeObject.prototype._editItem=function(c){if(this._eItEd){this._stopEdit();var a=this._globalIdStorageFind(c);if(a){editText=this.callEvent("onEdit",[0,c,this,a.span.innerHTML]);if(editText===!0)editText=a.span.innerText||a.span.textContent;else if(editText===!1)return;this.dADTempOffEd=this.dADTempOff;this.dADTempOff=!1;this._editCell=a;a._oldValue=editText;a.span.innerHTML="<input type='text' class='intreeeditRow' />";a.span.style.paddingRight=a.span.style.paddingLeft="0px";a.span.onclick=
a.span.ondblclick=function(a){(a||event).cancelBubble=!0};a.span.childNodes[0].value=editText;a.span.childNodes[0].onselectstart=function(a){return(a||event).cancelBubble=!0};a.span.childNodes[0].onmousedown=function(a){return(a||event).cancelBubble=!0};a.span.childNodes[0].focus();a.span.childNodes[0].focus();a.span.onclick=function(a){(a||event).cancelBubble=!0;return!1};a.span.className="";a.span.parentNode.className="";var b=this;a.span.childNodes[0].onkeydown=function(a){if(!a)a=window.event;
if(a.keyCode==13)a.cancelBubble=!0,b._stopEdit(window.undefined);else if(a.keyCode==27)b._editCell.span.childNodes[0].value=b._editCell._oldValue,b._stopEdit(window.undefined);(a||event).cancelBubble=!0};this.callEvent("onEdit",[1,c,this])}}};
